...
const {isLoaded, isSignedIn} = useUser();
  ...
  const onSubmit = async (data) => {
    try {
      if (!user) {
        showDialog("error", "Error", "You must be logged in to create a blog post.");
        return;
      }

      const uploader_name = isLoaded && isSignedIn && user?.username || user?.fullName || "Anonymous";
      const uploader_email = isLoaded && isSignedIn && user?.primaryEmailAddress?.emailAddress || "Anonymous";
      const uploader_avatar = isLoaded && isSignedIn && user?.imageUrl || "Anonymous";
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8 sm:mb-10 flex flex-col items-center justify-center">
          ...
          <p className="text-muted-foreground mt-3 text-base sm:text-lg text-center max-w-2xl">
            Fill in the details below to create a new blog post with rich media content
          </p>
        </div>
          ...
          <Card className="mt-6 shadow-md border-border">
            ...
            <CardContent className="px-5 sm:px-6 lg:px-8 pb-8">
              <Form {...form}>
                ...
                >
                  {/* ── Content Tab ──────────────────────────────────────── */}
                  <TabsContent value="content" className="mt-0 space-y-6 sm:space-y-8">
                    <FormField
                      control={form.control}
                      name="title"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-base sm:text-lg font-medium">Title (Required)</FormLabel>
                          <FormControl>
                            <Input placeholder="Enter a compelling title for your post" {...field} className="w-full text-base sm:text-lg py-3" />
                          </FormControl>
                          <FormDescription className="text-xs sm:text-sm">
                            Make it catchy and descriptive. This will be the main headline.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-base sm:text-lg font-medium">Sub-Heading (Required)</FormLabel>
                          <FormControl>
                            <Textarea placeholder="Write a brief summary of your post (will appear in previews)" className="min-h-[80px] sm:min-h-[100px] resize-y w-full text-base" {...field} />
                          </FormControl>
                          <FormDescription className="text-xs sm:text-sm">
                            A short description that will appear in blog listings and meta descriptions.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-base sm:text-lg font-medium">Content (Required)</FormLabel>
                          <FormControl>
                            <Textarea placeholder="Write your blog post content here..." className="min-h-[250px] sm:min-h-[350px] lg:min-h-[450px] resize-y w-full text-base" {...field} />
                          </FormControl>
                          <FormDescription className="text-xs sm:text-sm">
                            Write the main content of your blog post. Supports Markdown formatting.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  {/* ── Media & Tags Tab ─────────────────────────────────── */}
                  <TabsContent value="media" className="mt-0 space-y-6 sm:space-y-8">
                    <FormField
                      control={form.control}
                      name="mediaFiles"
                      render={({ field }) => (
                        <FormItem>
                                ...
                                >
                                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                    ...
                                    <p className="text-xs sm:text-sm text-muted-foreground text-center">
                                      Images: PNG, JPG, GIF, WEBP (MAX. 5MB each)
                                    </p>
                                    <p className="text-xs sm:text-sm text-muted-foreground text-center">
                                  ...
                                  <input id="media-upload" type="file" className="hidden" accept="image/*,video/*" multiple onChange={handleMediaUpload} />
                                </label>
                              </div>
                                ...
                                <div className="w-full">
                                  ...
                                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                                    {mediaItems.map((item, index) => (
                                      <div key={index} className="relative group rounded-lg border overflow-hidden aspect-square">
                                        {item.type === "image" ? (
                                          ...
                                          <div className="relative w-full h-full flex items-center justify-center bg-black/10">
                                            <video src={item.preview} className="max-h-full max-w-full" controls />
                                          </div>
                                        )}
                                        <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs p-1 truncate">
                                          {item.name}
                                        </div>
                                        <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2 h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleRemoveMediaItem(index)}>
                                          <X className="h-3 w-3" />
                                        </Button>
                                      </div>
                                    ))}
                          ...
                          <FormDescription className="text-xs sm:text-sm mt-3">
                            Upload images and videos to enhance your blog post. First image will be used as the featured image.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-base sm:text-lg font-medium">Tags</FormLabel>
                          <FormControl>
                            <div className="space-y-4">
                              <div className="flex gap-2">
                                <Input placeholder="Add a tag and press Enter" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyPress={handleTagKeyPress} className="flex-1 text-base" />
                              ...
                              {tags.length > 0 && (
                                <div className="flex flex-wrap gap-2 min-h-[40px] p-3 border border-input rounded-md">
                                  {tags.map((tag, index) => (
                                    <Badge key={index} variant="secondary" className="flex items-center gap-1 px-3 py-1.5 text-sm">
                                      <Tag className="h-3 w-3" />
                                      {tag}
                                      <Button type="button" variant="ghost" size="icon" className="h-5 w-5 ml-1 hover:bg-transparent" onClick={() => handleRemoveTag(tag)}>
                                        <X className="h-3 w-3" />
                                      </Button>
                                    </Badge>
                                  ))}
                          ...
                          <FormDescription className="text-xs sm:text-sm">
                            Add relevant tags to help users find your content. Maximum 10 tags.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
                    <div className="flex gap-3 w-full sm:w-auto">
                      <Button type="button" variant="outline" onClick={() => { form.reset(); setMediaItems([]); }} className="flex-1 sm:flex-none text-sm sm:text-base py-2.5">
                        Clear Form
                      </Button>
                      <Button type="submit" disabled={form.formState.isSubmitting} className="flex-1 sm:flex-none text-sm sm:text-base py-2.5">
